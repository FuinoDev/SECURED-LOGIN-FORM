...
import { getLockoutUntil, hashPassword, PasswordValidationError, verifyPassword, } from "./password.service.js";
import { createPasswordResetToken, createSessionToken, createVerificationToken, getPasswordResetExpiryDate, getSessionExpiryDate, getVerificationExpiryDate, hashProvidedToken, } from "./token.service.js";
...
export async function registerUser(input, meta) {
    ...
    if (existing) {
        return {
            message: "If this email is available, a verification link has been sent. Check your inbox to continue.",
        };
    }
    let passwordHash;
    try {
...
export async function logoutUser(sessionToken, meta, userId) {
    if (sessionToken) {
...
export async function resendVerificationEmail(emailInput, meta) {
...
export async function requestPasswordReset(emailInput, meta) {
    const email = normalizeEmail(emailInput);
    const user = await prisma.user.findUnique({ where: { email } });
    const genericMessage = {
...
export async function changePassword(userId, input, meta, currentSessionToken) {
